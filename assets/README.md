...
```
assets/
├── pix-qrcode.png      → QR Code PIX (raiz de assets/, não tem subpasta própria)
...
Sem `pix-qrcode.png`, `videos/*.jpg` ou `og-image.jpg`, a página continua funcionando normalmente — só falta o visual (QR Code e miniaturas aparecem quebrados até a imagem existir). Os blocos de Patrocinador Master, Patrocinadores e Apoiadores ficam **escondidos automaticamente** enquanto não houver logo cadastrado (ver [CONFIG.md](../CONFIG.md#patrocinadores--patrocinadores)), então não aparece nada quebrado nessa seção.
