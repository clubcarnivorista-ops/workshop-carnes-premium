...
| `videos/*.jpg` (opcional) | Miniatura customizada de um vídeo (`item.thumb` em `VIDEO_GROUPS`, `script.js`). **Não é obrigatório**: se `thumb` ficar vazio, a miniatura é buscada automaticamente do próprio YouTube (`img.youtube.com/vi/ID/hqdefault.jpg`) — hoje nenhum vídeo cadastrado usa arquivo local | 640×360px (16:9) | JPG, até 150KB |
...
Sem `og-image.jpg`, a página continua funcionando normalmente — só falta o visual do preview de compartilhamento. As miniaturas dos vídeos **não dependem de nenhum arquivo em `videos/`** — vêm do próprio YouTube automaticamente, então nunca aparecem quebradas. Cada categoria de `PATROCINADORES.categorias` (Parceiros Oficiais, Patrocinador Master, Patrocinadores Ouro, Patrocinadores Prata) fica **escondida automaticamente** enquanto não tiver nenhum item cadastrado (ver [CONFIG.md](../CONFIG.md#patrocinadores--patrocinadores)), então não aparece nada quebrado nessa seção também.
